    ...
    <section class="public-panel panel">
      ...
      <template v-else-if="activity">
        ...
        <el-form :model="formValues" label-position="top" @submit.prevent="submit">
          <el-form-item v-for="field in fields" :key="field.field_key" :label="field.field_label" :required="field.required">
            <el-input
            ...
            />
            <el-input v-else-if="field.field_type === 'textarea'" v-model="formValues[field.field_key]" type="textarea" :rows="4" :placeholder="field.placeholder" />
            <el-select v-else-if="field.field_type === 'select'" v-model="formValues[field.field_key]" :placeholder="field.placeholder">
              <el-option v-for="option in field.options_json" :key="option" :label="option" :value="option" />
            </el-select>
            <el-select v-else-if="field.field_type === 'multi_select'" v-model="formValues[field.field_key]" multiple :placeholder="field.placeholder">
              <el-option v-for="option in field.options_json" :key="option" :label="option" :value="option" />
            </el-select>
            <el-radio-group v-else-if="field.field_type === 'radio'" v-model="formValues[field.field_key]">
              <el-radio v-for="option in field.options_json" :key="option" :label="option">{{ option }}</el-radio>
            </el-radio-group>
            <el-checkbox v-else-if="field.field_type === 'checkbox'" v-model="formValues[field.field_key]">{{ field.placeholder || '确认' }}</el-checkbox>
            <div v-if="field.help_text" class="caption">{{ field.help_text }}</div>
          </el-form-item>
          <el-button type="primary" :loading="submitting" class="submit-button" @click="submit">提交报名</el-button>
        </el-form>
      </template>
    </section>
  </main>
...
async function submit() {
  for (const field of fields.value) {
    ...
    if (field.required && (value === '' || value === undefined || (Array.isArray(value) && !value.length))) {
      ElMessage.error(`请填写 ${field.field_label}`)
      return
    }
  }
